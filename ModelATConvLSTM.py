...
class DotProductAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        score = torch.bmm(query, key.transpose(1, 2)) / (self.hidden_dim ** 0.5)
...
class ConvLSTMDecoderWithAttention(nn.Module):
    def __init__(self, out_channels, hidden_dims):
        super(ConvLSTMDecoderWithAttention, self).__init__()
        ...
        self.attention = DotProductAttention(hidden_dim=hidden_dims[2])
        self.query_transform = nn.Linear(hidden_dims[2], hidden_dims[2])  # Transform hidden states to query
        self.value_transform = nn.Linear(hidden_dims[2], hidden_dims[2])  # Transform hidden states to value
    ...
    def decode(self, z, future_seq, hidden_states):
        ...
        for step in range(future_seq):
            ...
            query = self.query_transform(h_t3).unsqueeze(1)  # Shape (batch_size, 1, hidden_dim) ---- make sure that it macthes your shape
            if outputs:
                value = torch.stack(outputs, dim=1) 
            else:
                value = query
            context, attn_weights = self.attention(query, value, value)
            all_attention_weights.append(attn_weights)

            h_t3 = h_t3 + context.squeeze(1)
            outputs.append(h_t3)

            print(f"Step {step + 1} attention weights:", attn_weights)

        outputs = torch.stack(outputs, dim=2)
        return torch.sigmoid(self.output_conv(outputs)), all_attention_weights
...
# Our BetaVAE model --- make sure your in_channels -- match your no.of channels 

class BetaVAEAttention(nn.Module):
    def __init__(self, in_channels, latent_dim, hidden_dims=[32, 64, 128]):
        super(BetaVAEWithAttention, self).__init__()
        
        self.encoder = ConvLSTMEncoder(in_channels, hidden_dims)
        self.decoder = ConvLSTMDecoderWithAttention(in_channels, hidden_dims)
    ...
    def forward(self, x, future_seq=10):
        ...
        outputs, attention_weights = self.decoder.decode(z, future_seq, decoder_hidden_states)
        return outputs, mu, logvar, attention_weights