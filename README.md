...
![Beta-Decode Framework: The architecture of \name~consists of two main stages: training and inference. The training phase has two modules: (i) a modal-specific noise coupling strategy applied to clean signals (x), (ii) followed by a dot-product attention-based beta-variational autoencoder module to learns the local and global temporal representation of the noisy signals and reconstructs the denoised signals. During inference phase, the trained model is applied to unseen Raw signals to separate noise and reconstruct denoised signals.](Beta_Decode.png)
